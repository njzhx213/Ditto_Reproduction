...
**Thirteen modules, verified bottom-up:**
...
- **Pipelined PE** (`pe_diff_pipe.v`): 3-stage (mul/sum/acc) version; same result after a 3-cycle drain, with `valid` flowing through the pipeline so bubbles add nothing. Synthesis showed this did *not* raise Fmax (the pipeline split the wrong logic) — see the carry-save PE and the synthesis section.
- **Carry-save PE** (`pe_diff_csa.v`): accumulator kept in carry-save form (no per-cycle carry propagation), resolving once at the output. This targets the real Fmax bottleneck the timing report exposed and reaches **3021 MHz, a 5x improvement** over the carry-propagate PEs, while staying functionally identical (resolved `acc` == numpy).
...
**Verification methodology** (see `docs/rtl_diagrams.md` for the datapath block diagram and the verification hierarchy): each level — unit, datapath, system, array, real-trace — is checked against the Functional Ditto / numpy golden reference. Honest notes: the >4-bit/wide rate depends on the (dynamic) quant scale as flagged throughout; the 4x4 array is a representative tile, not the full 39398-PE fabric; and FPGA synthesis was run (Vivado 2025.1, ZU3EG, out-of-context). The three carry-propagate PE variants all top out at ~600 MHz (`pe_diff` 599.9, `pe_diff_pipe` 575.7, `pe_diff_slot` 612.4), and the timing report shows the common critical path is the **32-bit accumulator carry chain** (5x CARRY8), not the multiply/sum logic the pipeline split — so pipelining gave nothing (even -4%, the wrong path). Acting on that diagnosis, a **carry-save accumulator** (`pe_diff_csa`, no per-cycle carry propagation) reaches **3021 MHz — a 5x improvement** — while staying functionally identical (resolved `acc` == numpy). The end-to-end lesson (documented in `SYNTHESIS.md`): profile the critical path before optimizing; a pipeline on the wrong path did nothing, a carry-save accumulator on the right path gave 5x. (3021 MHz is an OOC estimate; P&R would lower it and the 4-lane multiply/sum tree becomes the next limiter — stated as a caveat.)
...
All RTL targets pass under cocotb + Icarus Verilog: `make` (encoding_unit), `x4`, `pe`, `pipe`, `csa`, `datapath`, `datapath_b`, `datapath_slot`, `defo`, `diffgen`, `vpu`, `array`, `array_sys`, `top`, `real_sdm`.
